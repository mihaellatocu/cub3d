...
/*   Updated: 2025/04/01 19:57:24 by mtocu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_rgb_value(int *rgb)
{
	...
	while (++k < 3)
	{
		if (rgb[k] < 0 || rgb[k] > 255)
			return (ft_printf("Error: RGB value %d out of range\n", rgb[k]), 0);
	}
	return (1);
}
...
int	parse_rgb_value(int *rgb, char *line, int i, int j)
{
	while (j < 3)
	{
		...
		if (!ft_isdigit(line[i]))
			return (ft_printf("Error: expected digit at pos %d\n", i), 0);
		...
		while (line[i] && ft_isspace(line[i]))
			i++;
		if (j < 2 && line[i++] != ',')
			return (ft_printf("Error: expected comma at pos %d\n", i - 1), 0);
		j++;
	}
	while (line[i] && ft_isspace(line[i]))
		i++;
	if (line[i])
		return (ft_printf("Error: extra char after RGB: '%s'\n", &line[i]), 0);
	if (!check_rgb_value(rgb))
		return (0);
	return (1);
}
...
void	parse_rgb_map(t_map *map, int *rgb, char *line)
{
	...
	while (*ptr && ft_isspace(*ptr))
		ptr++;
	if (!parse_rgb_value(rgb, ptr, 0, 0))
		parse_err(map, "Error\nRGB line is incorrect\n");
}
